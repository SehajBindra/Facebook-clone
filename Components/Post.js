    ...
    <div className="flex flex-col">
      ...
      <div className="flex justify-between items-center  bg-white shadow-lg text-gray-500 border-t">
        ...
        <div className="InputIcon">
          ...
          <p className="text-xs sm:text-sm md:first-letter:text-base font-medium">
            {comments.length > 0 && (
              <p className=" font-semibold mb-1"> {comments.length} Comments</p>
            )}
          </p>
        </div>
        <div className="InputIcon ">
      ...
      {session && (
        // {comments.length > 0 && (
        <div className="  bg-white max-h-8 overflow-y-scroll scrollbar-hide ">
          {comments.map((comment) => (
            <div
              className=" flex items-center justify-start space-x-2 mb-6 ml-5 "
              key={comment.id}
            >
              <img
      ...
      {session && (
        <form className="flex items-center p-4 bg-white rounded-b-2xl">
          <EmojiHappyIcon className="h-8 mr-3 text-yellow-300" />
          <input
            type="text"
            value={comment}
